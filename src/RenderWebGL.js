...
class RenderWebGL extends EventEmitter {
     ...
     * @returns {!int} the ID for the checker.
     */
    createEditTargetBox (editTarget,targetDrawable) {
    ...
    updateEditTargetBoxSkin (editTarget){
        ...
        if(!editTarget.isStage){
            const drawable = this._allDrawables[editTarget.drawableID];
            this.createEditTargetBox(editTarget,drawable);
        }

        const costumeSize = this.getSkinSize(editTarget.drawableID);
     ...
     */
    setEditTargetBoxVisible (visible) {
    ...
    createCheckerSkin () {
        const skinId = this._nextSkinId++;
        const newSkin = new CheckerSkin(skinId, this,this._xLeft,this._xRight,this._yBottom,this._yTop);
        this._allSkins[skinId] = newSkin;
        return skinId;
    }
    /**
     ...
     * @returns {!int} the ID for the new skin.
     */
    createEditTargetBoxSkin (editTarget,targetDrawable) {
        const skinId = this._nextSkinId++;
        const newSkin = new EditTargetBoxSkin(skinId, this, editTarget,targetDrawable);
        this._allSkins[skinId] = newSkin;
        return skinId;
    }
    /**
